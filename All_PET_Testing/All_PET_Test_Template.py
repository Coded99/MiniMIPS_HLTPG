...
def pseudo_template(inputFile, out, instruct, result_register):
 ...
 # def load_data_immediate(i, register, opcode):-- Com by Ade
     # bit_set1 = i[:16]
     # bit_set2 = i[16:32]
     # bit_set3 = i[32:48]
     # bit_set4 = i[48:68]
  ...
  #def load_data_shift(i, register, opcode): -- comm by Ade
     # bit_set1 = i[:16]
    #  bit_set2 = i[16:32]
    #  bit_set3 = i[32:48]
    #  bit_set4 = i[48:68]
  ...
  #def alu_shifts(file_name, offset, opcode): -- Comm by Ade
     # for i in (line):
 ...
 # def alu_immediate(file_name, offset, opcode): --Comm by Ade
      #for i in (line):
 ...
 # def alu_op(file_name, offset): --comm by Ade
      #for i in (line):
...
#  def hi_lo(file_name, offset,opcode):-- Comm by me surely
    #  for i in (line):
         # load_data_shift(i, register,opcode)
         # if(opcode[2:4]=="hi"):
            #  out.write("\tmfhi $%d\n" % (register+2)) 
 ...
 # elif((instruction == "mtlo") or (instruction =="mthi")):
     # hi_lo(f,offset,instruction) --Comm starts at elif and ends here
  else:
      alu_op(f,offset)
  ...
  for line in Ins:
    if "=" not in line:
      try:
        ...
        instruction = instr[0:]
        if (catergory == 'p_'):
          #if (instruction == 'addu' or instruction == 'add'):--my commenting starts here
